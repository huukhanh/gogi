...
ROLE_RE='^(coordinator|dev|techlead|po|investigator(-[0-9]+)?|user|all)$'
KIND_RE='^(brief|memo|consult|answer|decision \[(small|big)\]|question-relay|review|report|status)$'

[[ "$SENDER" =~ $ROLE_RE ]] || { echo "log.sh: unknown sender '$SENDER' (use coordinator|dev|techlead|po|investigator-N|user)" >&2; exit 2; }
