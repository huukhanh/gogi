...
Agents cannot see their own token usage, so it is collected from the transcripts on disk: `${CLAUDE_PLUGIN_ROOT}/scripts/session-stats.sh "$RUN"` reads the session's main transcript and every `subagents/agent-*.jsonl`, dedupes streaming duplicates by message id, and writes `$RUN/session.json` + `$RUN/session.md`: **one row per spawned agent for the whole session, finished, stopped or rotated alike** (named after the `name:` given at spawn, read from the transcript's `.meta.json`; unnamed helpers appear as `type@id`), every token column a sum over all of that agent's turns, plus the last-turn **Context** size, a `⚠ rotate` flag when a live agent is over budget (a rotated predecessor is marked `(rotated)` and never re-flagged), a totals row, and a **per-role table across generations** (`dev → dev-2 → dev-3`, summed) so the cost of a role for the task is one number. The monitor's `watch.sh` runs it **on every tick**; the coordinator runs it **once more at the very end** before the final report and quotes the totals line. Agent rows appear only after that agent has produced its first message.
